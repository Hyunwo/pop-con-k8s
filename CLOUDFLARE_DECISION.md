...
| **월 합계** | **이벤트 횟수에 따라 변동 (하단 표 참고)** | **$0** | **$20 (고정)** |

### 트래픽별 AWS WAF 비용 변화

> **트래픽 추정 근거**: 순간 최대 동시접속 15,000명 기준, 대기열 polling(5초 간격) 포함 시
> 이벤트 1회당 약 300만~540만 요청 발생. 월 이벤트 횟수에 따라 300만~2,000만 요청으로 변동될 것으로 예상됩니다.
> 실제 수치는 서비스 오픈 후 Grafana(Prometheus) 모니터링으로 측정할 예정입니다.

AWS WAF는 요청량에 비례해 비용이 증가합니다. Cloudflare는 트래픽량과 무관하게 고정 요금입니다.

| 월 요청 수 | 예상 이벤트 횟수 | AWS WAF 요청 비용 | AWS WAF 합계 (ACL + 룰 5개 + 요청) | Cloudflare Pro |
|-----------|--------------|-----------------|-----------------------------------|---------------|
| 300만 | ~1회 | $1.80 | **$11.80** | $20 |
| 600만 | ~2회 | $3.60 | **$13.60** | $20 |
| 1,200만 | ~4회 | $7.20 | **$17.20** | $20 |
| **1,700만** | **~5회** | **$10.20** | **$20.20** ← 역전 | $20 |
| 2,400만 | ~8회 | $14.40 | **$24.40** | $20 |

> 월 약 **1,700만 요청(이벤트 약 5회)** 을 초과하면 AWS WAF 비용이 Cloudflare Pro를 역전합니다.
